...
/* LOADING PANEL */ const h4Elements = document.querySelectorAll(".loading-title");
...
/* NAV BUTTON COLOR */ const main = document.querySelector("main");
...
main.addEventListener("scroll", ()=>{
    ...
    sections.forEach((section, index)=>{
        ...
        if (scrollTop >= section20Percent && scrollTop < section80Percent) paintButtonNav(index);
        if (scrollTop >= sectionStart - windowHeight / 2 && scrollTop < sectionMiddle) activeSection = index;
    });
    if (activeSection !== null) paintButtonNav(activeSection);
});
function paintButtonNav(activeIndex) {
    navButtons.forEach((button, index)=>{
        if (index == activeIndex) button.classList.add("navSelected");
        else button.classList.remove("navSelected");
    });
}
/* DINAMIC TEXT - SECTION 1 */ var phrases = [
    "Web Developer...",
    "Front-end Developer...",
    "Back-end Developer..."
];
...
var intervalo = setInterval(function() {
    ...
    letterIndex++;
    if (letterIndex === phrases.length) letterIndex = 0;
    setTimeout(function() {
        deletePhrase();
    }, fraseActual.length * 50 + 1000);
}, 5000);
function writePhrase(fraseActual) {
    for(var i = fraseActual.length; i >= 0; i--)(function(i) {
        setTimeout(function() {
...
function deletePhrase() {
    for(var j = 0; j <= phrases[letterIndex].length; j++)(function(j) {
        setTimeout(function() {
            h2Element.textContent = phrases[letterIndex].substring(0, j);
        }, j * 50);
    })(j);
}
/* CAROUSEL - SECTION 2 */ const carousels = document.querySelectorAll(".carousel");
...
function updateCarousel() {
    ...
    carousels.forEach((carousel, i)=>{
        ...
        if (i === carouselIndex) carousel.classList.add("active");
...
/* SECTION 4 */ const cardBoxes = document.querySelectorAll(".card-box");
...
/* FORM CONTACT ME */ const form_name = document.querySelector("#name");
